<?php
  ...
  $meta['info']['title']='Purchase Report by Supplier';
...
function check_access_level_radio(clas)
{
  ...
  //                                              $balance_status=gnrm($db,'ledger',$contact_where,'balance_type');
                              ...
                              <div class="table-responsive m-t-40" id="no-more-tables">
                                  ...
                                  $select_qry="SELECT * FROM stock_history sh right JOIN purchase_invoices si  on sh.invoice_id=si.id where  sh.owner_mobile = '$_SESSION[sess_bp_username]' and sh.in_out='purchase' $where ";
//                                    echo $select_qry;
...
//                                        echo '<h2>'.$this_num.' '.htmlspecialchars($_GET['c_num']).'</h2><hr />';
                                    ...
                                    if($count>0)
                                    {
                                      ...
                                      <thead>
                                        <tr>
                                          <th>Customer</th><th>Product</th><th>Date</th><th>Qty</th><th>Unit Price</th><th>Total Price</th><th>Unit</th>
                                        </tr></thead><tbody>';
                                      foreach($h_rows as $h_row)
                                      {
                                        $contact_id=gnrm($db,'contacts',"`owner_mobile`='$_SESSION[sess_bp_username]' and `number`='$h_row[contact_number]'",'id');
                                        if(array_key_exists($h_row['contact_number'],$contacts_data) && array_key_exists($h_row['product_id'],$products_data))
                                        {
                                          ...
                                          echo '<tr><td><a href="su-contacts-gallery.php?id='.$contact_id.'">'.$contacts_data[$h_row['contact_number']]['name'].' '.$h_row['contact_number'].'</a></td><td><a href="su-products-gallery.php?product_id='.$h_row['product_id'].'">'.$products_data[$h_row['product_id']]['name'].'</a></td><th>'.$h_row['date'].'</th><th>'.$h_row['qty'].'</th><th>'.$h_row['unit_price'].'</th><th>'.$h_row['total_price'].'</th><th>'.$h_row['measuring_unit'].'</th></tr>';
                                        }
                                      }
                                      echo '</tbody></table>';
                                      ?>
