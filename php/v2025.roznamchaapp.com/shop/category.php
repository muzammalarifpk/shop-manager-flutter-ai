...
$sql_all_products = "select count(*) from `products` where `owner_mobile`='$shop_' and `category`='$get_explode[2]' and `platforms` like '%moqame%' and `status`='Published' order by `name` ";
// echo $sql_all_products;
...
$sql_products = "select * from `products` where `owner_mobile`='$shop_' and `category`='$get_explode[2]'  and `platforms` like '%moqame%'  and `status`='Published' order by `name` limit $start_limit,$items_per_page ";
...
if($products_count>0)
{
  if ($product_data = $stmt_products->fetch())
  {
    do
    {
      ...
      $wheres = "`ref_id`='$product_data[id]' and `status`='Published' and `type`='products'";

      $image_url = gnrm($db,'gallery',$wheres,'file_path');
...
<!-- Product Catagories Area Start -->
          ...
          foreach ($products_data_array as $key => $product)
          {
            ...
            <div class="single-products-catagory clearfix">
                <a href="<?=$product['url']?>">
                    ...
                    <!-- Hover Content -->
                    <div class="hover-content" style="background-color: rgba(0,0,0,.5); padding: 5px 10px; top:0; left:0;">
                        <div class="line"></div>
                        <p style="color: #ffffff; ">Price <?=$user_data['currency']?><?=$product['price']?></p>
                        <h4 style="color: #ffffff; "><?=$product['title']?></h4>
                    </div>
                </a>
            </div>
            <?php
    ...
    <div id="loadmore"><a href="" class="btn">Load More</a></div>

    <section class="slider-pagination" style="display:none" role="navigation">
