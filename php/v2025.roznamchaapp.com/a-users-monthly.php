<?php
  ...
  $meta['info']['title']='User Monthly Stats';
                        ...
                        <div class="card">
                            <div class="card-body">
                              ...
                              <?php
                                ...
                                $select_first_qry="SELECT `timestamp`  FROM `users` order by `timestamp` asc limit 1";
                                $stmt_first_user = $db->query($select_first_qry);
                                $row_first_user = $stmt_first_user->fetch(PDO::FETCH_ASSOC);
                                $first_month = strtotime(date("01-m-Y",$row_first_user['timestamp']));

                                $select_last_qry="SELECT `timestamp`  FROM `users` order by `timestamp` desc limit 1";
                                $stmt_last_user = $db->query($select_last_qry);
                                $row_last_user = $stmt_last_user->fetch(PDO::FETCH_ASSOC);
                                $last_month = strtotime(date("30-m-Y",$row_last_user['timestamp']));

                                $month_start = $first_month;

                                while($month_start<$last_month)
                                {
                                  $month_end = mktime(0, 0, 0, date('m',$month_start)+1, 1, date('Y',$month_start));;


                                  $qry_select_free="select count(*) as free_users from `users` where `timestamp`>'$month_start' and `timestamp`<'$month_end' and `type`='prepaid' ";
                                  $stmt_free = $db->query($qry_select_free);
                                  $row_free = $stmt_free->fetch();

                                  $qry_select_premium="select count(*) as premium_users from `users` where `timestamp`>'$month_start' and `timestamp`<'$month_end'  and `type`!='prepaid' ";
                                  $stmt_premium = $db->query($qry_select_premium);
                                  $row_premium = $stmt_premium->fetch();

                                  $qry_select_1_entries="select count(*) as user_count from `users` where `timestamp`>'$month_start' and `timestamp`<'$month_end'  and `entries`>'0' ";
                                  $stmt_1_entries = $db->query($qry_select_1_entries);
                                  $row_1_entries = $stmt_1_entries->fetch();

                                  $qry_select_5_entries="select count(*) as user_count from `users` where `timestamp`>'$month_start' and `timestamp`<'$month_end'  and `entries`>'5' ";
                                  $stmt_5_entries = $db->query($qry_select_5_entries);
                                  $row_5_entries = $stmt_5_entries->fetch();

                                  $qry_select_25_entries="select count(*) as user_count from `users` where `timestamp`>'$month_start' and `timestamp`<'$month_end'  and `entries`>'25' ";
                                  $stmt_25_entries = $db->query($qry_select_25_entries);
                                  $row_25_entries = $stmt_25_entries->fetch();

                                  $qry_select_50_entries="select count(*) as user_count from `users` where `timestamp`>'$month_start' and `timestamp`<'$month_end'  and `entries`>'50' ";
                                  $stmt_50_entries = $db->query($qry_select_50_entries);
                                  $row_50_entries = $stmt_50_entries->fetch();

                                  $qry_select_100_entries="select count(*) as user_count from `users` where `timestamp`>'$month_start' and `timestamp`<'$month_end'  and `entries`>'100' ";
                                  $stmt_100_entries = $db->query($qry_select_100_entries);
                                  $row_100_entries = $stmt_100_entries->fetch();

                                  $month_name = date("Y-m M",$month_start);
                                  $cohorts_array[$month_name]=array(
                                    'cohort'=>$month_name,
                                    'total_users'=>$row_free['free_users']+$row_premium['premium_users'],
