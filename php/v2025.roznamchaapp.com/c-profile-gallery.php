...
$insert_qry = "insert into `gallery` set `owner_mobile` = :owner_mobile, `timestamp`=:timestamp,  `added_by`=:added_by,  `status`=:status,  `last_updated`=:last_updated, `ref_id`=:ref_id, `type`=:type,  `file_path`=:file_path,  `file_name`=:file_name";
...
try
{
  ...
  if (!empty($_FILES)) {
       ...
       $status = 'Published';
       $type = 'logo';
       $ref_id = floatval($_SESSION['sess_bp_username']);
       ...
       {
         $update_logo_qry="update `users` set `logo`='$target_file' where `number`='$_SESSION[sess_bp_username]'";
         $stmt_update_logo_qry=$db->prepare($update_logo_qry);

         $stmt_update_logo_qry->execute();

         $_SESSION['sess_bp_logo']=$target_file;
