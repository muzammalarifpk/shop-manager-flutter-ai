                        ...
                        <div class="card m-t-20">
                            <div class="card-body">
                              ...
                              $i=1;
                                ...
                                $select_qry="select * from `todo` where `owner_mobile`='$_SESSION[sess_bp_username]' $all_status_where order by `job` asc";
...
<?php                                    foreach ($db->query($select_qry) as $row) {
                                   ...
                                   ?>
                                    <tr class="item_row" data-rowid="<?=$row['id']?>">
                                      <td data-title="" class="bolder">
                                        <input type="checkbox" name="items[]" value="<?=$row['id']?>" id="item_<?=$row['id']?>" style=" position: relative; left: 0; opacity: 100;">
                                        <a href="" class="editmodalbtn" rel="<?=$row['id']?>"><?=$row['job']?></a>
                                      </td>
                          ...
                          <br>
                          <p>
                            <a href="#" class="btn btn-sm pull-right btn-success" id="addtodo2sale">Add to Invoice</a>
                            <a href="su-todo.php?show_all_status=yes" class="btn btn-sm pull-right btn-info">Show Deleted</a>
                            <a href="su-todo.php" class="btn btn-sm pull-right btn-inverse">Hide Deleted</a>
                          </p>
                          <?php
                        } ?>
  ...
  <script type="text/javascript">
    ...
    $(document).on('click','#editbtn',function(e){
      ...
      var urlpost = 'su-todo-edit.process.php';
    ...
    $(document).on('click','#submitBtn',function(e){
      ...
      $('.preloader').show();
      var form_data = $("#new_form").serialize();
      var urlpost='su-todo-new.process.php';
    ...
    $(document).on('click','#addtodo2sale',function(e){
      var checkedtodo = [];
      $( ".item_row input" ).each(function( index ) {
        if($(this).prop("checked") == true)
        {
          var this_value = $( this ).val();
          checkedtodo.push(this_value);
          console.log( index + ": " + $( this ).val() );
        }
        console.log(checkedtodo);

        var new_url = 't-wholesale.php?todo_items='+checkedtodo;
        console.log(new_url);
