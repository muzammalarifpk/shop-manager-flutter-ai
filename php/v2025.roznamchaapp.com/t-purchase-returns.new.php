...
$purchase_qry = "insert into `purchase_invoices_returns` set `owner_mobile` = :owner_mobile, `timestamp`=:timestamp,  `added_by`=:added_by,  `status`=:status,  `last_updated`=:last_updated,  `cartitems`=:cartitems,  `contact_number`=:contact_number,  `date`=:date,  `sub_total`=:sub_total,  `discount`=:discount,  `grand_total`=:grand_total,  `amount_paid`=:amount_paid,  `payment_method`=:payment_method,  `location_id`=:location_id,  `remaining_amount`=:remaining_amount, `notes`=:notes ";
...
$credit_array=array(array('account'=>$purchases_account,'amount'=>$_REQUEST['sub_total']));
$debit_array=array(array('account'=>'c'.$_REQUEST['cname'],'amount'=>$_REQUEST['remaining_balance']),array('account'=>$_REQUEST['payment_method'],'amount'=>$_REQUEST['amount_paid']),array('account'=>$purchases_discount_account,'amount'=>$_REQUEST['discount']));


$entry_type='purchase_return: '. $_REQUEST['notes'];
...
foreach($cart_items as $key => $val)
{
  ...
  update_stock_on_location($db,$val['item_id'],$val['row_qty'],'purchase_return',$_REQUEST['location_id']);
}
...
update_stock_history($db,$purchaseid,$_REQUEST['date'],'purchase_return',$products_array);
