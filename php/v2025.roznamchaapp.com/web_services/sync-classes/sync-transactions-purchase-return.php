...
function store_transaction_purchaseReturn($db,$owner_mobile,$inputData,$deviceID)
{
  $response=[];

  foreach($inputData as $record)
  {
    $response[$record['_id']]=process_single_transactionPurchaseReturn($db,$owner_mobile,$record,$deviceID);
  }
...
function process_single_transactionPurchaseReturn($db,$owner_mobile,$postinfo,$deviceID)
{

  $_SESSION['sess_bp_username']=$owner_mobile;
  $table='purchase_invoices_returns';
  ...
  if(count($err)==0)
  {
    ...
    {
      $response = insert_single_transaction_PurchaseReturn($db,$owner_mobile,$postinfo,$deviceID,$table);
...
function insert_single_transaction_PurchaseReturn($db,$owner_mobile,$postinfo,$deviceID,$table)
{
  ...
  if(is_array($postinfo['cartItems']))
  {
    ...
    foreach($postinfo['cartItems'] as $key => $val)
    {
      ...
      update_stock_on_location($db,$val['item_id'],$val['qty'],'purchase_return',$location_id);
    }
  }
  ...
  $purchases_account=gnrm($db,'chartofaccount',"`owner_mobile`='$owner_mobile' and `account_head`='purchases' and `status`='Published'",'id');
  ...
  $entry_type='purchase_return: '. $postinfo['notes'];
  ...
  $credit_array1=array(array('account'=>$purchases_account,'amount'=>$postinfo['subTotal']),array('account'=>$tax_account,'amount'=>$postinfo['tax']));
  //$journal1=journal_entry($db,$credit_array1,$debit_array1,$entry_type,$entry_link);


  $debit_array2=array(array('account'=>$purchase_discount_account,'amount'=>$postinfo['discountAmount']),array('account'=>$postinfo['paymentMethod'],'amount'=>$postinfo['amountReceived']));
  ...
  try
  {
    ...
    $entry_link='purchase_return_id:'.$saleid;
  ...
  //          print_r($lend_row);
    ...
    {
      update_stock_history($db,$saleid,$postinfo['date'],'purchase_return',$products_array);
    }
    if(count($variants_array)>0)
    {
      foreach ($variants_array as $key => $value) {
        // code...
        update_stock_variant_history($db,'purchase_return_invoice:'.$saleid,$postinfo['date'],'purchase_return',$value);
      }
    }
    ...
    $response['code']=200;
    $response['msg']='Transcation Purchase Return created successfully';
...
function update_single_transaction_PurchaseReturn($db,$owner_mobile,$postinfo,$deviceID,$table)
{
  $empty='';
...
function get_data2sync_Purchase_Return($db,$owner_mobile,$deviceID)
{
  if($owner_mobile)
  {
    $time = time();
    $qry = "select * from `purchase_invoices_returns` where `owner_mobile`='$owner_mobile' and  (`sync` is NULL || `sync` NOT LIKE '%$deviceID%' )";
    $stmt = $db->prepare($qry);
    ...
    try{
      ...
      if($count>0)
      {
        ...
        $response['code'] = 200;
        $response['msg'] = 'Transcation purchase returns fetched successfully.';
        $response['count']=count($Sales);
        }else{
        $response['code'] = 201;
        $response['msg'] = 'No Transcation purchase returns Found.';
      }
    }
    catch(PDOException $e)
    {
