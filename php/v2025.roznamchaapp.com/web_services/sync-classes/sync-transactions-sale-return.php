...
function store_transaction_sale_return($db,$owner_mobile,$inputData,$deviceID)
{
  $response=[];

  foreach($inputData as $record)
  {
    $response[$record['_id']]=process_single_transactionSale_return($db,$owner_mobile,$record,$deviceID);
  }
...
function process_single_transactionSale_return($db,$owner_mobile,$postinfo,$deviceID)
{

  $_SESSION['sess_bp_username']=$owner_mobile;
  $table='sale_invoices_returns';
  ...
  if(count($err)==0)
  {
    ...
    {
      $response = insert_single_transaction_Sale_return($db,$owner_mobile,$postinfo,$deviceID,$table);
...
function insert_single_transaction_Sale_return($db,$owner_mobile,$postinfo,$deviceID,$table)
{
  ...
  if(is_array($postinfo['cartItems']))
  {
    ...
    foreach($postinfo['cartItems'] as $key => $val)
    {
      ...
      update_stock_on_location($db,$val['item_id'],$val['qty'],'sale_return',$location_id);
    }
  }
  ...
  $insert_stmt->bindParam('service_count', $empty); // extra
  $insert_stmt->bindParam('cart_items_services', $empty); // extra
  ...
  $insert_stmt->bindParam('attachments', $attachments); // extra
  ...
  $entry_type='sale_return: '. $postinfo['notes'];
  ...
  //          print_r($lend_row);
    ...
    {
      update_stock_history($db,$saleid,$postinfo['date'],'sale_return',$products_array);
    }
    if(count($variants_array)>0)
    {
      foreach ($variants_array as $key => $value) {
        // code...
        update_stock_variant_history($db,'sale_return:'.$saleid,$postinfo['date'],'sale_return',$value);
      }
    }
    ...
    $response['code']=200;
    $response['msg']='Transcation Sale Return created successfully';
...
function update_single_transaction_Sale_return($db,$owner_mobile,$postinfo,$deviceID,$table)
{
  $empty='';
...
function get_data2sync_Sale_return($db,$owner_mobile,$deviceID)
{
  if($owner_mobile)
  {
    $time = time();
    $qry = "select * from `sale_invoices_returns` where `owner_mobile`='$owner_mobile' and  (`sync` is NULL || `sync` NOT LIKE '%$deviceID%' )";
    $stmt = $db->prepare($qry);
    ...
    try{
      ...
      if($count>0)
      {
        ...
        $response['code'] = 200;
        $response['msg'] = 'Transcation Sale Return fetched successfully.';
        $response['count']=count($Sales);
        }else{
        $response['code'] = 201;
        $response['msg'] = 'No Transcation Sale Return Found.';
      }
    }
    catch(PDOException $e)
    {
