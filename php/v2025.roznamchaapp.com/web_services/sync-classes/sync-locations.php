...
function store_stockLocations($db,$owner_mobile,$inputData,$deviceID)
{
  $response=[];

  foreach($inputData as $record)
  {
    $response[$record['_id']]=process_single_stockLocations($db,$owner_mobile,$record,$deviceID);
  }
...
function process_single_stockLocations($db,$owner_mobile,$postinfo,$deviceID)
{
  ...
  if(empty($postinfo['name'])){
    $err[]='Locations Name is Required.';
  }
  ...
  if(count($err)==0)
  {
    ...
    {
      $response[] = insert_single_stockLocations($db,$owner_mobile,$postinfo,$deviceID,$table);
    //
    ...
    $response=update_single_stockLocations($db,$owner_mobile,$postinfo,$deviceID,$table);
...
function insert_single_stockLocations($db,$owner_mobile,$postinfo,$deviceID,$table)
{
  $insert_qry = "insert into `$table` set `name`=:name,  `type`=:type,  `description`=:description,  `tags`=:tags,  `notes`=:notes, `status`=:status, `last_updated`=:last_updated,  `owner_mobile`=:owner_mobile,  `added_by`=:added_by,  `timestamp`=:timestamp,  `sync`=:sync ";
  ...
  $insert_stmt->bindParam('type', $postinfo['types']);
  ...
  $response['code']=200;
  $response['msg']='stockLocations created successfully';
...
function update_single_stockLocations($db,$owner_mobile,$postinfo,$deviceID,$table)
{
  $empty='';
  ...
  $update_qry="update `$table` set `status`='$status',`last_updated`='$last_updated', `sync`='$sync', `name`='$name', `description`='$description',  `notes`='$notes', `tags`='$tags',  `type`='$type' where `owner_mobile`='$owner_mobile' and `id`='$id'";
  ...
  $response['code']=200;
  $response['msg']='Locations updated successfully.';
...
function get_data2sync_stockLocations($db,$owner_mobile,$deviceID)
{
  if($owner_mobile)
  {
    $time = time();
    $qry = "select * from `locations` where `owner_mobile`='$owner_mobile' and  (`sync` is NULL || `sync` NOT LIKE '%$deviceID%' )";
    $stmt = $db->prepare($qry);
    ...
    try{
      ...
      if($count>0)
      {
        ...
        $response['code'] = 200;
        $response['msg'] = 'stockLocations fetched successfully.';
        $response['count']=count($stockLocations);
        }else{
        $response['code'] = 201;
        $response['msg'] = 'No stockLocations Found.';
      }
    }
