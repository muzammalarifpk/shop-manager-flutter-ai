...
function store_chartOfAccount($db,$owner_mobile,$inputData,$deviceID)
{
  $response=[];

  foreach($inputData as $record)
  {
    $response[$record['_id']]=process_single_chartOfAccount($db,$owner_mobile,$record,$deviceID);
  }
...
function process_single_chartOfAccount($db,$owner_mobile,$postinfo,$deviceID)
{
  ...
  if(empty($postinfo['account_head'])){
  ...
  if(count($err)==0)
  {
    ...
    {
      $response[] = insert_single_chartOfAccount($db,$owner_mobile,$postinfo,$deviceID,$table);
    //
    ...
    $response=update_single_chartOfAccount($db,$owner_mobile,$postinfo,$deviceID,$table);
...
function insert_single_chartOfAccount($db,$owner_mobile,$postinfo,$deviceID,$table)
{
  $insert_qry = "insert into `$table` set `account_head`=:account_head,  `account_type`=:account_type,  `balance`=:balance,  `balance_type`=:balance_type,  `old_balance`=:old_balance,  `old_balance_type`=:old_balance_type,  `last_update_date`=:last_update_date, `notes`=:notes, `status`=:status, `last_updated`=:last_updated,  `owner_mobile`=:owner_mobile,  `added_by`=:added_by,  `timestamp`=:timestamp,  `sync`=:sync ";
  ...
  $insert_stmt->bindParam('account_head', $postinfo['account_head']);
  $insert_stmt->bindParam('account_type', $postinfo['account_type']);
  $insert_stmt->bindParam('balance', $postinfo['balance']);
  $insert_stmt->bindParam('balance_type', $postinfo['balance_type']);
  $insert_stmt->bindParam('old_balance', $postinfo['old_balance']);
  $insert_stmt->bindParam('old_balance_type', $postinfo['old_balance_type']);
  $insert_stmt->bindParam('last_update_date', $postinfo['last_update_date']);
  ...
  $response['code']=200;
  $response['msg']='chart of account created successfully';
...
function update_single_chartOfAccount($db,$owner_mobile,$postinfo,$deviceID,$table)
{
  $empty='';
  ...
  $account_head=$postinfo['account_head'];
  $account_type=$postinfo['account_type'];
  $balance=$postinfo['balance'];
  $balance_type=$postinfo['balance_type'];
  $old_balance=$postinfo['old_balance'];
  $old_balance_type=$postinfo['old_balance_type'];
  $last_update_date=$postinfo['last_update_date'];
  ...
  $update_qry="update `$table` set `status`='$status',`last_updated`='$last_updated', `sync`='$sync', `account_head`='$account_head', `account_type`='$account_type',  `balance`='$balance', `balance_type`='$balance_type', `old_balance`='$old_balance' , `old_balance_type`='$old_balance_type', `last_update_date`='$last_update_date', `notes`='$notes' where `owner_mobile`='$owner_mobile' and `id`='$id'";
...
function get_data2sync_chartOfAccounts($db,$owner_mobile,$deviceID)
{
  if($owner_mobile)
  {
    $time = time();
    $qry = "select * from `chartofaccount` where `owner_mobile`='$owner_mobile' and  (`sync` is NULL || `sync` NOT LIKE '%$deviceID%' )";
    $stmt = $db->prepare($qry);
