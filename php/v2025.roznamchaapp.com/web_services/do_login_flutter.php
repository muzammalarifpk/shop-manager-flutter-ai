    ...
    try {
      ...
      $row   = $stmt->fetch(PDO::FETCH_ASSOC);
        ...
        if($count!==1)
        {
          ...
          $token = store_token($db,$row['number'],$deviceID);
          if($token)
          {
            $default_logo = 'uploads/images/default-logo.png';

            $logo_url = 'https://shop-manager.roznamchaapp.com/'.$row['logo'];
            if(!file_exists($logo_url))
            {
              $logo_url = 'https://shop-manager.roznamchaapp.com/'.$default_logo;
            }
            $response['code']=200;
            $response['msg']='Login successfully.';
            $response['data']=array('id'=>$row['id'],
            'token'=>$token,
            'username'=>$row['number'],
            'addedBy'=>$row['number'],
            'name'=>$row['business_name'],
            'account_keys'=>$row['default_account_keys'],
          ...
          ,'privs'=>'*');
            ...
            $response['code']=210;
            $response['msg']='Login Success but failed to store... '.$token;
