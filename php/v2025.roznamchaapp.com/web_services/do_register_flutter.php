...
function store_token($db,$owner_mobile,$deviceID)
{
  $length = 64;
  ...
  $select_old_token_qry="select count(*) from `user_access` where `owner_mobile` = ? and `deviceID` = ? ";

  $stmt = $db->prepare($select_old_token_qry);
  $stmt->execute([$owner_mobile, $deviceID]);
  $count = $stmt->fetchColumn();

  if(intval(trim($count))!==0)
  {
    $update_token_qry="update `user_access` set `token`=:token, `expiry`=:expiry, `timestamp`=:timestamp, `ip`=:ip, `access_logs`=:access_logs, `access_header`=:access_header where `owner_mobile`=:owner_mobile and `deviceID`=:deviceID ";
    ...
    $token_qry = "insert into `user_access` set `owner_mobile` = :owner_mobile, `status`=:status,  `ip`=:ip,  `token`=:token, `deviceID`=:deviceID, `timestamp`=:timestamp, `expiry`=:expiry,  `access_logs`=:access_logs,  `access_header`=:access_header";
...
}elseif(!isJson($JSON2))
{
  ...
  }else
  {
    ...
    $fields['req']=array('business_name','country_code','mobile','number','password');
    ...
    if(!empty($err))
    {
      ...
      $response['code']=204;
      $response['msg']='Ops Validation Error: '.$err_html;
  ...
  if(!$stmt)
  {
    ...
    if(!$update_stmt)
    {
      ...
      $token = store_token($db,$owner_mobile,$deviceID);



      /******************** Your code ***********************/
      $reply_data['id']   = $user_id;
      $reply_data['username'] = $owner_mobile;
      $reply_data['token'] = $token;
      $reply_data['addedBy'] = $owner_mobile;
      $reply_data['name'] = $JSON2['business_name'];
      $reply_data['account_keys'] = gnr($db,'users','id',$user_id,'default_account_keys');
      $reply_data['industry_type'] = $JSON2['industry_type'];
      $reply_data['business_type'] = $JSON2['business_type'];
      $reply_data['address'] = '';
      $reply_data['country_code'] = $JSON2['country_code'];
      $reply_data['mobile'] = $JSON2['mobile'];
      $reply_data['currency'] = $currency;
      $reply_data['gst'] = '';
      $reply_data['vat'] = '';
      $reply_data['negative'] = 'off';
      $reply_data['tax'] = 'off';
      $reply_data['secondary_units'] = 'off';
      $reply_data['variants'] = 'off';
      $reply_data['barcode'] = 'off';
      $reply_data['logo'] = 'https://shop-manager.roznamchaapp.com/'.$default_logo;
      $reply_data['salesman_commission'] = 'off';
      $reply_data['agent_commission'] = 'off';
      $reply_data['print_header_note'] = '';
      $reply_data['print_footer_note'] = '';
