...
//        echo $key.' - '.trim($postinfo[$key]);
      ...
      if($value['type']=='manual')
      {
        if($key=='privs')
        {
            ...
            $privs=$postinfo['access_level_select'];

            if($postinfo['access_level_select']=='*')
            {
              $privs='*';
            }else {
              if(!empty($postinfo['privs']))
              {
                $privs='';
                foreach ($postinfo['privs'] as $manualkey => $manualvalue) {
                  $privs.=$manualvalue.',';
                }
              }else{
                $err.='<li>Please select atleast one module access.</li>';
              }
            }
  ...
  if(isset($_GET['reqid']))
  {
    process_insert_form($db,$_POST,$all_fields,'admin',$_GET['reqid']);
  }else{
    process_insert_form($db,$_POST,$all_fields,'admin');
  }
