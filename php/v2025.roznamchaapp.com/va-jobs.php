...
function check_access_level_radio(clas)
{
  ...
  $(document).ready(function(e){
    ...
    $(document).on('click','.job_summary',function(e){
      e.preventDefault();
      $('.preloader').show();
      var reqid=$(this).attr('rel');
        $.get( "<?=$meta['module'][1]?>-summary.php?reqid="+reqid, function( data ) {
          // the contents is now in the variable data
          $('#modaldiv').html(data);
          $('#responsive-modal').modal('show');
          $('.preloader').hide();
                        ...
                        <div class="card">
                            <div class="card-body">
                              <div class="row">
                                ...
                                <?php
                                  if(isset($_GET['tag']))
                                  {
                                    ?>
                                    <div class="col-md-6">
                                        <h5>Tag: <a href="va-jobs.php" class="btn btn-danger"><i class="fa fa-close"></i> <?=$_GET['tag']?></a></h5>
                                    </div>
                              ...
                              if(isset($_GET['tag']))
                              {
                                ...
                                $select_qry="select * from `va-jobs` where `owner_mobile`='$_SESSION[sess_bp_username]' $all_status_where order by `name` asc";
                              ...
                              <div class="table-responsive m-t-10"  id="no-more-tables">
                                <table id="example23" class="display nowrap table table-hover table-striped table-bordered" cellspacing="0" width="100%">
                                    <thead class="cf">
                                      <tr>
                                        <th>Name</th>
                                        <th class="hide_it">Tags</th>
                                        <th class="numeric">Number</th>
                                        <th class="numeric">Total Output</th>
                                  ...
                                  <?php
                                   ...
                                   ?>
                                    <tr class="">
                                      ...
                                      $tags= explode(',',substr($row['tags'],2,-2));

                                      foreach($tags as $tag)
                                      {
                                        if($tag)
                                        {
                                          echo '<a href="va-jobs.php?tag='.$tag.'" class="btn btn-sm btn-default">'.$tag.'</a>';
                                        }
                                      }
                                      ?></td>
                                      <td data-title="Number: "><?=$row['number']?></td>
                                      <td data-title="Total: "><?=$row['total_output']?></td>
                                      <td data-title="">
                                        <?=$row['status']?>
                                        <a href="" class="editmodalbtn pull-right btn btn-sm btn-warning" rel="<?=$row['id']?>">Edit</a>
                                        <a href="" class="pull-right job_summary btn btn-sm btn-success" rel="<?=$row['id']?>">Summary</a>
                                        <a href="va-jobs-new-activity.php?job_id=<?=$row['id']?>" class="pull-right btn btn-sm btn-primary" rel="<?=$row['id']?>">Add Activity</a>
                                      </td>
                          ...
                          <br>
                          <p>
                            <a href="va-jobs.php?show_all_status=yes" class="btn btn-sm pull-right btn-info">Show Deleted</a>
                            <a href="va-jobs.php" class="btn btn-sm pull-right btn-inverse">Hide Deleted</a>
                          </p>
                          <?php
                          } ?>
                            </div>
  ...
  <script type="text/javascript">
    ...
    $(document).on('click','#editbtn',function(e){
      ...
      var urlpost = 'va-jobs-edit.process.php';
      $('.preloader').show();
    ...
    $(document).on('click','#submitBtn',function(e){
      ...
      var urlpost='va-jobs-new.process.php';
